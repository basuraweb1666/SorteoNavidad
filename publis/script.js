...
const now = new Date();
const drawDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 21, 55, 00).getTime();
...
async function drawOfficialWinner() {
  try {
    ...
    if (!res.ok) throw new Error('Error generando ganador');
    const data = await res.json();
    console.log('Número ganador generado:', data.winner);
    return data.winner;
  } catch (e) {
...
function selectWinner() {
    ...
    // Usar el número ganador oficial (ya obtenido del servidor)
